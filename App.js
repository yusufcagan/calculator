...
function App () {
  ...
  const onChange = (cNumber) => {
    ...
    if(cNumber == '+' || cNumber == '=' || cNumber == '-' || cNumber == '/' || cNumber == 'x' || cNumber == '%' || cNumber == 'del'){
      
      if(bool2 == true){
          switch (temp) {
          case '+':
            setNumber(sum1+parseFloat(sum2))
            break;
         ...
         if(cNumber == '=' || cNumber == '-' || cNumber == '/' || cNumber == 'x' || cNumber == '+'){
           setbool(false)
           setbool2(false)
           setsum1('')
           setsum2('')
  ...
  return (
    <View style={{flex:1,backgroundColor:'#000'}}>         
      ...
      <View style={{flexDirection:'row', width:width, justifyContent:'space-between'}}>
        ...
        <TouchableOpacity onPress={() => onChange('del')}><View style={styles.border}><Text style={{fontSize:30, color:'#b32400'}}>DEL</Text></View></TouchableOpacity>    
